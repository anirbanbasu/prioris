...
Prioris is a Claude Code CLI plugin, which you can use to search, fetch, and discuss prior art against the ideas you're working on. Deliberately scoped to **literature review and discussion only**; it will _**not**_ draft, outline, or write any part of a manuscript, that you may need to create, for you.
...
Discussion notes, quick-read sections, and quiz questions are all saved to persistent, taggable, per-paper storage via `prioris-mcp`'s `NotesBackend`, so your reading record accumulates across papers over time. Cross-paper recall already works today — `reading-log` searches your notes across every paper you've covered, and `quiz-me`'s review mode runs spaced repetition across everything you've quizzed, not just one paper. A graph layer over that same corpus is also available, as a set of explicit, user-invoked tools rather than automatic recall: `graph-backfill`, `graph-extract`, `manage-graph`, and `concept-explore` let you build and then traverse a concept graph across the papers and notes you've accumulated. There is still no vector index, so semantic/similarity search over your notes remains a future version.
...
- Cross-paper recall already works today through `NotesBackend`'s search and tagging — `reading-log` and `quiz-me`'s review mode both span every paper you've noted, not just one at a time. On top of that, `prioris-mcp`'s graph backend holds `Pointer` nodes for fetched documents and notes plus `Concept` nodes you extract from them, explored through `graph-backfill`/`graph-extract`/`manage-graph`/`concept-explore` — nothing is written to it without your approval, and nothing is read from it unless you ask. There is no vector index yet.
...
- `/prioris:graph-backfill [scope]` — backfill documents and notes that already exist but predate automatic structural graph sync into the graph, as `Pointer` nodes and `annotates` edges.
- `/prioris:graph-extract [paper, note, or @file]` — extract `Concept` nodes and relations from a document's or note's text into the graph, with alias-aware duplicate avoidance and explicit approval before every write.
- `/prioris:manage-graph` — add, edit, remove, and merge `Concept` nodes and relations by hand; also promotes a stub concept (from `rmotd`) to a real `Pointer` once the paper is fetched.
- `/prioris:concept-explore [scope and/or ask]` — traverse and analyze the graph: neighbors, paths, centrality, communities, reachability, link prediction, and whole-graph export/visualize.
...
Run `just sync` once (or let the first `uv run` in `.mcp.json` do it lazily) to create the `.venv` this repo's `pyproject.toml` describes, which pins the companion `prioris-mcp` server and provides the interpreter for `shared/scripts/` (see `shared/scripts/README.md`). Because `uv.lock` pins an exact git commit rather than re-resolving the tracked branch on every launch, `prioris-mcp` won't pick up new upstream commits automatically — run `just update-mcp` to re-pin to the latest commit when you want that.
...
The graph skills (`graph-backfill`, `graph-extract`, `manage-graph`, `concept-explore`) have a stronger dependency still: they need `prioris-mcp`'s `research_graph_query/write/analyze` tools and `research://graph/*` resources, which currently live only on that project's **unmerged** `feature/graph-backend` branch (upstream PR #46). They will not work against a released `prioris-mcp`, or against its `master`, until that PR merges and this repo's pin (below) is updated.

This plugin bundles a `.mcp.json` that launches `shared/scripts/mcp_stdio_proxy.py` via `uv run --project ${CLAUDE_PLUGIN_ROOT}`. That proxy is only a stdio-to-HTTP relay: the actual `prioris-mcp` server instance lives in a single canonical HTTP process (`shared/scripts/mcp_http_server.py`) that the relay starts on demand and that shuts itself down when idle, so exactly one process on the machine ever holds the graph database open — see `shared/scripts/README.md` and `docs/superpowers/plans/2026-09-21-single-mcp-server-lifecycle.md`. Both run against the local, version-pinned environment defined by the repo's own `pyproject.toml`/`uv.lock` (currently tracking `prioris-mcp`'s git `feature/graph-backend` branch — upstream PR #46 — pinned to whatever commit was last locked; this is a temporary pin for the graph backend prototype and is expected to revert to `master` once that PR merges, see the `# TEMP:` note on `[tool.uv.sources]` in `pyproject.toml` and "Local development" above). `uv run` creates that environment on first use, so there's still nothing to install ahead of time beyond `uv` itself on your `PATH`.
