...
6. **Record** — skip this step entirely if step 2/3 short-circuited to a cached section; it's already recorded. Otherwise append or update a "Quick read" section in `.prioris/discussions/<provider>/<identifier>.md` using the same seven headers, and set `read_at` if it isn't already set. Do this automatically — never ask the user whether to save it first. If a quick-read entry already exists for this paper (e.g. from an explicit redo), replace just that section rather than the whole file, so the rest of the notes aren't lost.
