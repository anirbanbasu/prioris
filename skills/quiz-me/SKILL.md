...
2. Check for an existing question bank: `research_notes_search(provider, canonical_identifier, tags_all=[<project_tag>, "skill:quiz-me", "type:question"])`.
   - Bank exists, and this isn't an explicit request to add/regenerate, and the paper hasn't been refetched to a newer version since the bank's oldest question was created → reuse it, skip to step 4.
   - Otherwise → generate (step 3). A refetch-to-a-newer-version case matters because anchors are exact-text quotes; a revised paper can shift or remove the quoted passage, silently invalidating the anchor.
