...
5. **Graph stub (optional)** — ask via `AskUserQuestion` once per digest whether the user wants any of these items tied into the graph (default: no — most `rmotd` runs are just browsing). For each item they choose to stub: `research_graph_write(op="create_concept", label=<title>, metadata={"stub_identifier": "<provider-or-openalex>:<identifier>"})` (see `../../shared/graph-model.md`'s stub-concept convention), each write confirmed individually first, same approval-every-time discipline `graph-extract` follows. Then attempt a lightweight relation-extraction pass off the abstract alone (reusing `graph-extract`'s find-or-create/approval flow, but flagging each written edge's `metadata` as abstract-derived — lower confidence than a full-text extraction). If the item isn't fetchable through this plugin (an OpenAlex "not fetchable here" entry with no `provider`/`identifier`), still create the stub, using `"doi:<doi>"` or `"pdf_url:<pdf_url>"` as the `stub_identifier` instead (whichever is present) — prefixed the same way as the `<provider>:<identifier>` convention, even though this kind of stub was never fetched through this plugin and so has no document `Pointer` to automatically verify against; promoting it later, if ever, is a manual `manage-graph` **Edit**, not the automatic promote flow.

6. **Go deeper** — if the user wants to go deeper on one listed item, hand off to `discuss` (or `quiz-me`); don't fetch full text here.
...
- `research_discovery(query, max_results?, page=1, from_year?, to_year?, open_access_only=false) -> {hits: [<DiscoveryHit>, ...], page, per_page, total, has_more}` — see `../../shared/mcp-contracts.md`'s "External discovery" section for the full contract, including the `DiscoveryHit.fetch_route` shape used in step 6 above.
