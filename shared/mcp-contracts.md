...
- `research_localfile_parse_full_text(id, offset=0, limit=None) -> {markdown, offset, limit, total_length, has_more, resource_uri}` — `id` is the caller-facing id `fetch_full_text`/`finalize_upload` returned, not `path`; fails `not_found` if that id was never returned by either. Paginated exactly like the other providers' `parse_full_text` — see "Paging through full text" below. No `format` choice exists for this source (always `pdf`).
