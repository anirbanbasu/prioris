...
- `research_localfile_fetch_full_text(path, format="pdf") -> {id, location, format, size_bytes, served_from_storage, resource_uri}` — `path` must be relative to the server's configured root and must not escape it; an absolute path is rejected outright (`invalid_request`). Fails `not_found` if nothing exists at that path, `file_too_large` if it exceeds the server's configured size ceiling, and `invalid_request` if the file's own bytes don't sniff as a PDF (checked by magic-byte prefix, not by trusting a `.pdf` extension). `id` is a caller-facing identifier this call mints (or, if the exact same file content was already fetched, the existing one) — opaque, not derived from `path`. Always re-reads and re-hashes the file fresh on every call, never cached at the MCP response layer, because the file can change on disk between calls without notice — never assume a previously-returned `id` is still current without calling this again first.
