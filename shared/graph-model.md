...
The server stores `relation_type` as free text (`GraphEdge.relation_type: str`, no server-side enum) and, unlike concepts (`find_concepts`/`research://graph/concepts`), has **no vocabulary-browsing aid for relation types** (an explicitly deferred follow-up per the backend's own ADR-00034) — vocabulary drift (`about` vs `is_about` vs `discusses`) has to be prevented here, by convention, not by querying the server. This doc's committed baseline is:
...
- `about` — `document → concept` or `note → concept`, the default relation `graph-extract` proposes between a fetched item and a concept it discusses. The user may override this per-edge during `graph-extract`'s approval step if a more specific relation_type fits better (e.g. `introduces`, `critiques`).

This baseline only changes via a deliberate, reviewed `graph-model.md` edit — never as a side effect of running a skill. A relation_type the user chooses at runtime that isn't in this baseline (a per-edge override) is recorded in the untracked working file `.prioris/graph-relation-vocabulary.json` instead (create it if absent; see `graph-extract`'s Round 3 for the exact read/append sequence), so repeat overrides stay consistent and de-duplicated across a working copy without touching version control. Promoting an override from that working file into this doc's committed baseline is a separate, deliberate edit the user asks for explicitly — not something any skill run does on its own.
