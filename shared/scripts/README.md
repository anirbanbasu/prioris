...
- **`manage_notes.py {search|read|create|update|delete} ...`** — deterministic CLI wrapper around `research_notes_*`: `search` loops pagination itself (`--all`) and can group results into one digest per paper (`--group-by-paper`), optionally attaching cached titles (`--attach-titles`); `read`/`create`/`update`/`delete` are thin 1:1 wrappers. Acts as its own in-process MCP client, like `pdf_chunk_upload_helper.py`. See `../notes-model.md` and `../mcp-contracts.md#notes`.
- **`manage_search.py search ...`** — deterministic CLI wrapper around `research_search_fetched`, looping pagination (`--all`) and applying a client-side `--min-score` floor (required practice for vector/hybrid mode, which is otherwise unthresholded).
- **`manage_discovery.py search ...`** — deterministic CLI wrapper around `research_discovery`, looping pages (`--all`) and adding two mechanical per-hit fields: `needs_condensation` (true past the 300-word abstract threshold `rmotd` uses) and, with `--include-work-types`, `work_type_name`/`work_type_description` resolved from `research://openalex/work-types`.
...
- **`_mcp_client.py`** — not invoked directly; shared in-process `fastmcp` client + validate-or-exit helpers (`call_tool`, `read_resource`) that every `manage_*.py` script above imports, factored out of what `pdf_chunk_upload_helper.py` originally did inline.
...
`pdf_chunk_upload_helper.py`'s own begin/chunk-loop/finalize sequence looks like the same shape but *is* scripted despite that reasoning, because it's the opposite case: nothing about an intermediate chunk's `upload_chunk` response needs to reach the conversation at all — only the final `finalize_upload` result does — so running the whole loop inside one script call, as its own MCP client, is strictly better than surfacing each chunk as a separate tool call the agent would otherwise have to make (and pay context for) one by one. The five `manage_*.py` scripts above follow the same reasoning for a different shape of repetition: pagination looping, cross-page aggregation, and fixed-vocabulary lookups (arXiv categories, OpenAlex work types) are all cases where only the final assembled list matters to the conversation, never an intermediate page - unlike `parse_full_text`'s loop, above, where each page's actual Markdown content is what the calling skill needs handed back.
